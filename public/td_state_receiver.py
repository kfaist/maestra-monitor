...
def _do_poll():
    ...
    try:
        ...
        # Find our entity by slug — pick the one with MOST state keys
        # (heartbeat creates empty-state duplicates every 2 min, burying real data)
        ...
        if matches:
            # Sort by: number of state keys (desc), then created_at (desc)
            # Use "or '0'" to handle None created_at — str(None) sorts wrong
            match = sorted(matches, key=lambda e: (
                len((e.get("state", {}) or {}).keys()),
                str(e.get("created_at") or "0")
            ))[-1]
        else:
            match = None
